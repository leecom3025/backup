...
import RT.HSystem;
import RT.Cryptography.CipherRT;
import RT.Cryptography.HMACRT;
import RT.Cryptography.KeyExchangeRT;
...
public class CommunicationSecurityProtocol 
{
	...
	private static int[] protocolHeaderLength=new int[]{32, -1, 0};
			...
			case 2:
				...
				{		
					case 0:
						...
						return new byte[][]{message};
					case Integer.MAX_VALUE:
						byte[][] plaintextMessages=new byte[(int)Math.ceil(((double)sendEncrypted.length)/blockSize)][];
						byte[] messageBytes;
						int messageInd=0;
						for(int byteIndex=0; byteIndex<sendEncrypted.length; byteIndex+=blockSize)
			            {
			                ...
			                System.arraycopy(sendUnencrypted[messageInd], 0, messageBytes, 0, sendUnencrypted[messageInd].length);
			                plaintextMessages[messageInd]=new byte[messageBytes.length];
			                System.arraycopy(messageBytes, 0, plaintextMessages[messageInd], 0, sendUnencrypted[messageInd].length);
							System.arraycopy(messageBytes, sendUnencrypted[messageInd].length, plaintextMessages[messageInd], sendUnencrypted[messageInd].length, messageBytes.length-sendUnencrypted[messageInd].length);
			                messageInd++;
			            }
