...
public class CannyEdgeDetector {
	...
	public void setSourceImage(byte[] imageData) {
	...
	private void follow(int x1, int y1, int i1, int threshold, int remainingDepth) {
		int x0 = x1 == 0 ? x1 : x1 - 1;
		...
		for (int x = x0; x <= x2; x++) {
			for (int y = y0; y <= y2; y++) {
				...
				if ((y != y1 || x != x1)
					...
					if(remainingDepth>0)
					{
						follow(x, y, i2, threshold, remainingDepth-1);
					}
					return;
				}
			}
