...
public class UI extends GameControl
{
	...
	protected Handler m_searchThreadUpdateHandler = new UIHandler(this);
...
class UIHandler extends Handler
{
	...
	public void handleMessage(Message msg) {
		...
		Log.i("searchThreadUpdatedler", "handle " + msg.what);
		// do move
		if(msg.what == ui.MSG_MOVE){

			ui.enableControl();
			int move = msg.getData().getInt("move");
			if(move == 0){
				ui.setMessage("No move found");
			} else
			{
				ui.move(move, "", true);
			}
		...
		else if(msg.what == ui.MSG_UCI_MOVE){
			ui.enableControl();
			ui.doUCIMove(msg.getData().getInt("from"), msg.getData().getInt("to"), msg.getData().getInt("promo"));
			//playNotification();
		}
		else if(msg.what == ui.MSG_TEXT){
			ui.setEngineMessage(msg.getData().getString("text"));
		}
		else if(msg.what == ui.MSG_PLAY){
			ui.play();
		}
