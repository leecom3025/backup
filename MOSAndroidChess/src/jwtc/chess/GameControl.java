...
public class GameControl extends ManagedObject
{
	...
	// human against PC
	public static int HUMAN_PC = 0;
	// PC against PC - self play or auto play
	// public static final int PC_PC = 2;
	
	public static final int LEVEL_TIME = 1;
	public static int LEVEL_PLY = 2;
	
	protected static Pattern _patNum = Pattern.compile("(\\d+)\\.");
	protected static Pattern _patAnnot = Pattern.compile("\\{([^\\{]*)\\}");
	protected static Pattern _patMove = Pattern.compile("(K|Q|R|B|N)?(a|b|c|d|e|f|g|h)?(1|2|3|4|5|6|7|8)?(x)?(a|b|c|d|e|f|g|h)(1|2|3|4|5|6|7|8)(=Q|=R|=B|=N)?(\\+|#)?([\\?\\!]*)?[\\s]*");
	protected static Pattern _patCastling = Pattern.compile("(O\\-O|O\\-O\\-O)(\\+|#)?([\\?\\!]*)?");
	...
	public void loadDB(InputStream in, String outFilename, int depth)
	{
		
		_openingDbFileName = outFilename;
		new LoadDBThread(this, in, outFilename, depth);
	...
	protected long getWhiteRemainClock(){
		long lDiff;
		if(_lClockStartWhite > 0)
		{
			lDiff=(System.currentTimeMillis() - _lClockStartWhite);
		}
		else
		{
			lDiff=0;
	...
	protected long getBlackRemainClock(){
		long lDiff;
		if(_lClockStartBlack > 0)
		{
			lDiff=(System.currentTimeMillis() - _lClockStartBlack);
		}
		else
		{
			lDiff=0;
	...
	public boolean move(int move, String sAnnotation, boolean bUpdate)
	{
	...
	public final boolean initFEN(String sFEN, boolean resetHead){
	...
	protected synchronized final boolean requestMove(String token, Matcher matchToken, String sCastle, String sAnnotation){
...
class LoadDBThread implements Runnable
{
	...
	public LoadDBThread(GameControl gc, InputStream in, String outFilename, int depth)
	{
		this.gc=gc;
		this.in=in;
	...
	public void run(){
		...
		try {
			...
			in.close();
			//}
			gc._jni.loadDB(gc._openingDbFileName, depth);
